...
typedef aclTensor* (*_aclCreateTensor)(const int64_t* view_dims, uint64_t view_dims_num,
                                       aclDataType data_type, const int64_t* stride, int64_t offset,
                                       aclFormat format, const int64_t* storage_dims,
                                       uint64_t storage_dims_num, void* tensor_data);
...
{
    if (!tensor.is_privateuseone()) { return true; }
...
inline void* GetOpApiFuncAddr(const char* apiName)
{
    ...
    if (opApiHandler == nullptr) { return nullptr; }
...
inline at::Tensor CopyTensorHostToDevice(const at::Tensor& cpu_tensor)
{
    ...
    int deviceIndex = 0;
    return cpuPinMemTensor.to(c10::Device(DEVICE_TYPE, deviceIndex), cpuPinMemTensor.scalar_type(),
                              true, true);
}
...
inline aclIntArray* ConvertType(const at::IntArrayRef& at_array)
{
    ...
    if (aclCreateIntArray == nullptr) { return nullptr; }
...
inline aclBoolArray* ConvertType(const std::array<bool, N>& value)
{
    ...
    if (aclCreateBoolArray == nullptr) { return nullptr; }
...
{
    if (opt_array.has_value()) { return ConvertType(opt_array.value()); }
    return nullptr;
}

inline aclScalar* ConvertType(const c10::optional<at::Scalar>& opt_scalar)
{
    if (opt_scalar.has_value()) { return ConvertType(opt_scalar.value()); }
    return nullptr;
}
...
inline void Release(aclTensor* p)
{
    ...
    if (aclDestroyTensor == nullptr) { return; }
    aclDestroyTensor(p);
}
...
inline void Release(aclScalar* p)
{
    ...
    if (aclDestroyScalar == nullptr) { return; }
    aclDestroyScalar(p);
}
...
inline void Release(aclIntArray* p)
{
    ...
    if (aclDestroyIntArray == nullptr) { return; }
...
inline void Release(aclBoolArray* p)
{
    ...
    if (aclDestroyBoolArray == nullptr) { return; }
...
inline void Release(aclTensorList* p)
{
    ...
    if (aclDestroyTensorList == nullptr) { return; }
...
#define EXEC_NPU_CMD(aclnn_api, ...)                                                               \
    do {                                                                                           \
        ...
        TORCH_CHECK(getWorkspaceSizeFuncAddr != nullptr && opApiFuncAddr != nullptr, #aclnn_api,   \
                    " or ", #aclnn_api "GetWorkspaceSize", " not in ", GetOpApiLibName(), ", or ", \
        ...
        auto acl_call = [converted_params, workspace_addr, workspace_size, acl_stream,             \
                         executor]() -> int {                                                      \
