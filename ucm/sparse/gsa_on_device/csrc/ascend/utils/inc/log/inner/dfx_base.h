...
private:
    ...
    {
        if (context == nullptr) { return "nil:nil"; }
    ...
    do {                                                                                         \
        if (AlogCheckDebugLevel(static_cast<int>(MOD_ID), (LOG_LEVEL)) == 1) {                   \
            AlogRecord(static_cast<int>(MOD_ID), DLOG_TYPE_DEBUG, (LOG_LEVEL),                   \
                       "[%s:%d][%s]%s[%s][%lu] OpName:[%s] " #FMT, __FILE__, __LINE__,           \
                       (OPS_UTILS_LOG_SUB_MOD_NAME), (OPS_UTILS_LOG_PACKAGE_TYPE), __FUNCTION__, \
...
#define OPS_LOG_STUB_FULL(LEVEL, OPS_DESC, FMT, ...)                                             \
    do {                                                                                         \
        if (0 == AlogCheckDebugLevel(OP, (LEVEL))) { break; }                                    \
        char msgbufxyz[ops::utils::LogBase::MAX_LOG_LEN];                                        \
        size_t msgmaxlen = (MSG_LENGTH - ops::utils::LogBase::MSG_HDR_LEN);                      \
        int rettmp =                                                                             \
            snprintf_s(msgbufxyz, sizeof(msgbufxyz), sizeof(msgbufxyz) - 1, FMT, ##__VA_ARGS__); \
        if (rettmp == -1) { msgbufxyz[sizeof(msgbufxyz) - 1] = '\0'; }                           \
